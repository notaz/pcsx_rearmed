...
/*
// Dirty workaround for the hardware problem (three keys pressed product four keys pressed for some keys combination like a+x+up arrow)
guint gsource = 0;
GdkEventKey key_event;

static gboolean
keys_handler(GtkWidget *widget)
{
	g_source_remove(gsource);
	gsource = 0;
	key_press_event(key_event.hardware_keycode, key_event.type == GDK_KEY_PRESS ? 1 : (key_event.type == GDK_KEY_RELEASE ? 2 : 0) );
	return TRUE;
}
*/
static void
...
{
	/*if (bGameGripperFix){
		//printf("Key: %i %s time %i \n", event->hardware_keycode, event->type == GDK_KEY_PRESS ? "Pressed" : (event->type == GDK_KEY_RELEASE ? "Released" : "Unknown"), event->time);

		//Trap the only case that happens with the game gripper:
		if (event->hardware_keycode != 33 && event->hardware_keycode != 111 && event->hardware_keycode != 38 && event->hardware_keycode != 53){
			return key_press_event(event->hardware_keycode, event->type == GDK_KEY_PRESS ? 1 : (event->type == GDK_KEY_RELEASE ? 2 : 0) );
		}else{
			if (gsource){
				g_source_remove(gsource);
				gsource = 0;
				//Never replace a key release event (some buttons may remain pressed)
				if (key_event.type == GDK_KEY_RELEASE || key_event.hardware_keycode != 33)
					key_press_event(key_event.hardware_keycode, key_event.type == GDK_KEY_PRESS ? 1 : (key_event.type == GDK_KEY_RELEASE ? 2 : 0) );
				else{
					//printf("Replacing Key: %i %s time %i \n", key_event.hardware_keycode, key_event.type == GDK_KEY_PRESS ? "Pressed" : (key_event.type == GDK_KEY_RELEASE ? "Released" : "Unknown"), key_event.time);
					//printf("         with: %i %s time %i \n", event->hardware_keycode, event->type == GDK_KEY_PRESS ? "Pressed" : (event->type == GDK_KEY_RELEASE ? "Released" : "Unknown"), event->time);
				}
			}

			//Set a timer to elaborate the key event:
			key_event = *event;
			gsource = g_timeout_add(40, (GSourceFunc)keys_handler, (gpointer)window);
		}
	}else{*/
		//printf("Key: %i %s time %i \n", event->hardware_keycode, event->type == GDK_KEY_PRESS ? "Pressed" : (event->type == GDK_KEY_RELEASE ? "Released" : "Unknown"), event->time);
		return key_press_event(event->hardware_keycode, event->type == GDK_KEY_PRESS ? 1 : (event->type == GDK_KEY_RELEASE ? 2 : 0) );
...
int maemo_init(int *argc, char ***argv)
{
	...
	//Check if the key mappings are for the Game Gripper to activate the "dirty workaround" :p
	if (direction_keys[0] == 38 && direction_keys[1] == 40 &&
		direction_keys[2] == 25 && direction_keys[3] == 53){
		bGameGripperFix = TRUE;
		printf("Game Gripper detected\n");
	}
...
void plat_trigger_vibrate(int is_strong)
{
	...
	if (last_vibrate && t1 - last_vibrate < vDuration / 1000.0){
		//printf("Don't vibrate (%f)\n", t1 - last_vibrate);
		return;
	}
	last_vibrate = t1;*/
